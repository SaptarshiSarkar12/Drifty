...
public class UpdateChecker {
    ...
    public static boolean isUpdateAvailable() {
        ...
        AppSettings.SET.setLastDriftyUpdateTime(System.currentTimeMillis());
    ...
    private static String getReleaseTag(HttpRequest request) {
        try (HttpClient client = HttpClient.newHttpClient()) {
            ...
            AppSettings.SET.setNewDriftyVersionName(responseBody.split("\"name\":\"")[1].split("\"")[0]);
            AppSettings.SET.setLatestDriftyVersionTag(tag);
    ...
    public static boolean compareVersions(String currentVersion, String latestVersion) {
        ...
        if (currentVersionParts.length == 3 && latestVersionParts.length == 5) {
            ...
            if (AppSettings.GET.isEarlyAccessEnabled()) { // If the user has enabled early access, then pre-release versions are considered
