...
<Tabs items={["GUI", "CLI"]}>
  <Tabs.Tab>
    <Tabs items={["Linux and Windows", "macOS"]}>
      <Tabs.Tab>
        {" "}
        - Run the below command to add access to the X server (required for GUI applications
        running in Docker) `shell xhost +local:docker ` - Run the below command to
        build and start the Drifty GUI native executable in a Docker container `shell
