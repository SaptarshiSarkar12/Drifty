...
2. Follow the below steps to run the project:
   - Open the `Drifty_CLI` Java class in the `CLI/src/main/java/main{:shell}` directory and click on the run button. This will start the Drifty CLI application.
   - Open the `Drifty_GUI` Java class in the `GUI/src/main/java/main{:shell}` directory and click on the run button. This will start the Drifty GUI application.
...
This will start the development server, and you can access the website at `http://localhost:3000{:bash}` in your browser.
...
To debug the website, you can use the browser's developer tools to inspect the elements, view console logs, and debug JavaScript code. You can also use the `console.log(){:js}` function to log messages to the console for debugging purposes.

## Contributing

If you would like to contribute to the project, please read the [Contributing Guidelines](../contribution.mdx) for more information on how to get started.
