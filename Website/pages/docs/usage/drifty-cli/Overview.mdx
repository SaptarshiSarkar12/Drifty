...
**Command**: `--version{:shell}` or `-v{:shell}`
...
**Command**: `--batch <file>{:shell}` or `-b <file>{:shell}`
...
**Command**: `--remove <index1> <index2> ...{:shell}` or `--remove all{:shell}`
...
**Command**: `--location <path>{:shell}` or `-l <path>{:shell}`
...
You can also use the `--location{:shell}` command in conjunction with this command.
</Callout>

**Command**: `--name <filename>{:shell}` or `-n <filename>{:shell}`
