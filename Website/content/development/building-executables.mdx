...
2.  Assuming you have installed the necessary project dependencies, run the below command to generate the C object file required only for building executable binaries for Drifty GUI
    ...
    <Tabs items={["Linux", "Windows", "macOS Aarch64", "macOS x86_64"]}>
      ...
      <Tabs.Tab>
        ```shell gcc -c config/missing_symbols.c -o
        config/missing_symbols-macos-14.o ```
      </Tabs.Tab>
      <Tabs.Tab>
        ```shell gcc -c config/missing_symbols.c -o
    ...
    <Tabs items={["GUI", "CLI"]}>
      <Tabs.Tab>
        <Tabs items={["Linux", "Windows", "macOS Aarch64", "macOS x86_64"]}>
          ...
          <Tabs.Tab>
            ```shell mvn -P build-drifty-gui-for-macos-14 gluonfx:build
            gluonfx:package -rf :GUI -U ```
          </Tabs.Tab>
          <Tabs.Tab>
            ```shell mvn -P build-drifty-gui-for-macos-13 gluonfx:build
            gluonfx:package -rf :GUI -U ```
          </Tabs.Tab>
        </Tabs>
      </Tabs.Tab>
      <Tabs.Tab>
        <Tabs items={["Linux", "Windows", "macOS Aarch64", "macOS x86_64"]}>
          ...
          <Tabs.Tab>
            ```shell mvn -P build-drifty-cli-for-macos-14 package ```
          </Tabs.Tab>
          <Tabs.Tab>
            ```shell mvn -P build-drifty-cli-for-macos-13 package ```
          </Tabs.Tab>
        </Tabs>
      </Tabs.Tab>
    </Tabs>
...
2.  Follow the below instructions to build and start the Drifty application in a Docker container
    ...
    <Tabs.Tab> 1. Run the below command to add access to the X server (required for GUI applications running in Docker)
    `shell
                xhost +local:docker
