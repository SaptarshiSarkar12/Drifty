...
2. Follow the below steps to run the project:
   - Open the `Drifty_CLI` Java class in the `CLI/src/main/java/main{:sh}` directory and click on the run button. This will start the **Drifty CLI** application.
   - Open the `Drifty_GUI` Java class in the `GUI/src/main/java/main{:sh}` directory and click on the run button. This will start the **Drifty GUI** application.
...
<Callout type="info">
   You need to run `npm run build{:sh}` before running the website in development mode for the first time to generate the required files for the search bar in documentation, to work properly.
</Callout>

This will start the development server, and you can access the website at [`http://localhost:3000{:sh}`](http://localhost:3000) in your browser.
...
To debug the website, you can use the browser's developer tools to inspect the elements, view console logs, and debug JavaScript code. You can also use the `console.log(){:sh}` function to log messages to the console for debugging purposes.
