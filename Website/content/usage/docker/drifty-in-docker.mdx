...
<Tabs items={["GUI", "CLI"]}>
  <Tabs.Tab>
    1. Open your terminal. 2. Pull the latest version of the Drifty Docker image
    by running the following command: ```sh docker pull
    ghcr.io/saptarshisarkar12/drifty-gui:latest ``` 3. If you are a **Linux**
    user or using **WSL2**, you may need to allow Docker to access your X
    server. You can do this by running the following command: ```sh xhost
    +local:docker ``` 4. Once the image is pulled, you can run it using the
    following command: ```sh docker run -e DISPLAY=$DISPLAY --net=host -v
    /tmp/.X11-unix:/tmp/.X11-unix ghcr.io/saptarshisarkar12/drifty-gui:latest
    ``` 5. Drifty GUI should now open on your screen.
  </Tabs.Tab>
  <Tabs.Tab>
    1. Open your terminal. 2. Pull the latest version of the Drifty Docker image
    by running the following command: ```sh docker pull
    ghcr.io/saptarshisarkar12/drifty-cli:latest ``` 3. Once the image is pulled,
    you can run it using the following command: ```sh docker run -it
    ghcr.io/saptarshisarkar12/drifty-cli:latest ```
  </Tabs.Tab>
</Tabs>
